...
class ClickAction extends BaseAction {
    ...
    async execute(page, selector) {
        try {
            ...
            throw new Error(`Click action failed for selector: ${selector}. ${error.message}`);
        }
    }

    /**
     * Click on an element in iframe
     * @param {Page} page - Playwright page object
    ...
    async executeInIframe(page, iframeSelector, elementSelector) {
        try {
            ...
            throw new Error(`Click action failed in iframe. Iframe: ${iframeSelector}, Element: ${elementSelector}. ${error.message}`);
        }
    }
}
